...
def extract_audio(video_path, path_to_audio_file_directory):
    ...
    if not path_to_audio_file_directory:
        logging.warning("Provided path to audio file does not exist.")
        folder_name = "extracted_audio"
        os.makedirs(folder_name, exist_ok=True)
        file_path = os.path.join(folder_name, base_name + ".wav")
    else:
    ...
    if os.path.exists(file_path):
        logging.info(
            f"Audio file already exists for the video {video_name}. Using the existing transcription to save resources.")
        return file_path
    ...
    try:
        ...
        logging.info(f"Successfully wrote the audio file.")
        return file_path
